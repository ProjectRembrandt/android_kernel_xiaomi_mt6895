#define PFX "REMBRANDTIMX582_camera_sensor"
...
#include "rembrandtimx582_ana_gain_table.h"
#include "rembrandtimx582_seamless_switch.h"
#include "rembrandtimx582mipiraw_Sensor.h"
...
static struct setting_workqueue *rembrandtimx582_workqueue = NULL;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = REMBRANDTIMX582_SENSOR_ID,
...
static kal_uint16 rembrandtimx582_QSC_setting[2304 * 2];
static kal_uint16 rembrandtimx582_LRC_setting_0[192 * 2];
static kal_uint16 rembrandtimx582_LRC_setting_1[192 * 2];



static kal_int32 async_write_cmos_sensor_8(struct subdrv_ctx *ctx,
                                 kal_uint16 *para, kal_uint32 len)
{
	u16 reg = para[0];
	u8 val  = para[1];

	subdrv_i2c_wr_u8(ctx, reg, val);
	IMX582_LOG_INF("reg(0x%04x) val(0x%02x)", reg, val);
...
static int write_cmos_sensor_8(struct subdrv_ctx *ctx, u16 reg, u8 val)
{
	...
	if (enable_async) {
		...
		rembrandtimx582_write_setting[1] = val;
		create_and_queue_setting_work(
					rembrandtimx582_workqueue,
					"async_write_cmos_sensor_8",
					EXP_SETTING,
					async_write_cmos_sensor_8,
					ctx,
					rembrandtimx582_write_setting,
					2);
	} else {
		ret = subdrv_i2c_wr_u8(ctx, reg, val);
	}
...
static void rembrandtimx582_get_pdaf_reg_setting(struct subdrv_ctx *ctx, MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void write_sensor_QSC(struct subdrv_ctx *ctx)
{
	...
	rembrandtimx582_burst_write_cmos_sensor(ctx, rembrandtimx582_QSC_setting,
		sizeof(rembrandtimx582_QSC_setting) / sizeof(kal_uint16));
...
static void write_sensor_LRC(struct subdrv_ctx *ctx)
{
	...
	rembrandtimx582_burst_write_cmos_sensor(ctx, rembrandtimx582_LRC_setting_0,
		sizeof(rembrandtimx582_LRC_setting_0) / sizeof(kal_uint16));

	rembrandtimx582_burst_write_cmos_sensor(ctx, rembrandtimx582_LRC_setting_1,
		sizeof(rembrandtimx582_LRC_setting_1) / sizeof(kal_uint16));
...
static void read_sensor_Cali(struct subdrv_ctx *ctx)
{
	...
	for (idx = 0; idx < 2304; idx++) {
		...
		rembrandtimx582_QSC_setting[2 * idx]     = sensor_qsc;
		rembrandtimx582_QSC_setting[2 * idx + 1] = read_cmos_eeprom_8(ctx, addr_qsc);
	}
	for (idx = 0; idx < 192; idx++) {
		rembrandtimx582_LRC_setting_0[2 * idx]     = sensor_lrc_0 + idx;
		rembrandtimx582_LRC_setting_0[2 * idx + 1] = read_cmos_eeprom_8(ctx, eeprom_lrc_0 + idx);
		rembrandtimx582_LRC_setting_1[2 * idx ]    = sensor_lrc_1 + idx;
		rembrandtimx582_LRC_setting_1[2 * idx + 1] = read_cmos_eeprom_8(ctx, eeprom_lrc_1 + idx);
	}
...
static void set_mirror_flip(struct subdrv_ctx *ctx, kal_uint8 image_mirror)
{
	...
	case IMAGE_NORMAL:
	subdrv_i2c_wr_u8(ctx, 0x0101, itemp);
	break;

	case IMAGE_V_MIRROR:
	subdrv_i2c_wr_u8(ctx, 0x0101, itemp | 0x02);
	break;

	case IMAGE_H_MIRROR:
	subdrv_i2c_wr_u8(ctx, 0x0101, itemp | 0x01);
	break;

	case IMAGE_HV_MIRROR:
	subdrv_i2c_wr_u8(ctx, 0x0101, itemp | 0x03);
	break;
	}
}
...
static kal_uint16 rembrandtimx582_feedback_awbgain[] = {
	0x0b8e, 0x00,
	0x0b8f, 0x00,
	0x0b90, 0x00,
	0x0b91, 0x00,
...
static kal_uint32 rembrandtimx582_awb_gain(struct subdrv_ctx *ctx,
		struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	...
	rembrandtimx582_feedback_awbgain[1] = (gr_gain_int & 0xFF00) >> 8;
	rembrandtimx582_feedback_awbgain[3] = gr_gain_int & 0xFF;
	rembrandtimx582_feedback_awbgain[5] = (r_gain_int & 0xFF00) >> 8;
	rembrandtimx582_feedback_awbgain[7] = r_gain_int & 0xFF;
	rembrandtimx582_feedback_awbgain[9] = (b_gain_int & 0xFF00) >> 8;
	rembrandtimx582_feedback_awbgain[11] = b_gain_int & 0xFF;
	rembrandtimx582_feedback_awbgain[13] = (gb_gain_int & 0xFF00) >> 8;
	rembrandtimx582_feedback_awbgain[15] = gb_gain_int & 0xFF;

	rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_feedback_awbgain,
		sizeof(rembrandtimx582_feedback_awbgain)/sizeof(kal_uint16));
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	wait_workqueue_done(rembrandtimx582_workqueue);
	enable_async = 0;
	IMX582_LOG_INF("setting done !\n");
...
static kal_int32 __rembrandtimx582_table_write_cmos_sensor(struct subdrv_ctx *ctx,
                                 kal_uint16 *para, kal_uint32 len)
{
	rembrandtimx582_table_write_cmos_sensor(ctx, para, len);

	return 0;
}

static kal_int32 rembrandtimx582_async_sensor_init(struct subdrv_ctx *ctx,
                                 kal_uint16 *para, kal_uint32 len)
{

	rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_init_setting,
		sizeof(rembrandtimx582_init_setting)/sizeof(kal_uint16));

	rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_Image_quality_setting,
                sizeof(rembrandtimx582_Image_quality_setting)/sizeof(kal_uint16));
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	if (enable_async) {
		create_and_queue_setting_work(
					rembrandtimx582_workqueue,
					"rembrandtimx582_init_setting",
					INIT_SETTING,
					rembrandtimx582_async_sensor_init,
					ctx,
					rembrandtimx582_init_setting,
					sizeof(rembrandtimx582_init_setting)/sizeof(kal_uint16));
	} else {
		rembrandtimx582_async_sensor_init(ctx, rembrandtimx582_init_setting,
			sizeof(rembrandtimx582_init_setting)/sizeof(kal_uint16));
	}
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	if (enable_async) {
		create_and_queue_setting_work(
					rembrandtimx582_workqueue,
					"rembrandtimx582_preview_setting",
					RES_SETTING,
					__rembrandtimx582_table_write_cmos_sensor,
					ctx,
					rembrandtimx582_preview_setting,
					sizeof(rembrandtimx582_preview_setting) / sizeof(kal_uint16));
	} else {
		rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_preview_setting,
			sizeof(rembrandtimx582_preview_setting)/sizeof(kal_uint16));
	}
...
static void normal_video_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	if (enable_async) {
		create_and_queue_setting_work(
					rembrandtimx582_workqueue,
					"rembrandtimx582_normal_video_setting",
					RES_SETTING,
					__rembrandtimx582_table_write_cmos_sensor,
					ctx,
					rembrandtimx582_normal_video_setting,
					sizeof(rembrandtimx582_normal_video_setting) / sizeof(kal_uint16));
	} else {
		rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_normal_video_setting,
			sizeof(rembrandtimx582_normal_video_setting)/sizeof(kal_uint16));
	}
...
static void hs_video_setting(struct subdrv_ctx *ctx)
{
	...
	if (enable_async) {
		create_and_queue_setting_work(
					rembrandtimx582_workqueue,
					"rembrandtimx582_hs_video_setting",
					RES_SETTING,
					__rembrandtimx582_table_write_cmos_sensor,
					ctx,
					rembrandtimx582_hs_video_setting,
					sizeof(rembrandtimx582_hs_video_setting) / sizeof(kal_uint16));
	} else {
		rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_hs_video_setting,
			sizeof(rembrandtimx582_hs_video_setting)/sizeof(kal_uint16));
	}
...
static void custom2_setting(struct subdrv_ctx *ctx)
{
	...
	if (enable_async) {
		create_and_queue_setting_work(
					rembrandtimx582_workqueue,
					"rembrandtimx582_custom2_setting",
					RES_SETTING,
					__rembrandtimx582_table_write_cmos_sensor,
					ctx,
					rembrandtimx582_custom2_setting,
					sizeof(rembrandtimx582_custom2_setting) / sizeof(kal_uint16));
	} else {
		rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_custom2_setting,
			sizeof(rembrandtimx582_custom2_setting)/sizeof(kal_uint16));
	}
...
static void custom3_setting(struct subdrv_ctx *ctx)
{
	...
	if (enable_async) {
		create_and_queue_setting_work(
					rembrandtimx582_workqueue,
					"rembrandtimx582_custom3_setting",
					RES_SETTING,
					__rembrandtimx582_table_write_cmos_sensor,
					ctx,
					rembrandtimx582_custom3_setting,
					sizeof(rembrandtimx582_custom3_setting) / sizeof(kal_uint16));
	} else {
		rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_custom3_setting,
			sizeof(rembrandtimx582_custom3_setting)/sizeof(kal_uint16));
	}
...
static void custom4_setting(struct subdrv_ctx *ctx)
{
	...
	rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_custom4_setting,
		sizeof(rembrandtimx582_custom4_setting)/sizeof(kal_uint16));
...
static void custom5_setting(struct subdrv_ctx *ctx)
{
	...
	rembrandtimx582_table_write_cmos_sensor(ctx, rembrandtimx582_custom5_setting,
		sizeof(rembrandtimx582_custom5_setting)/sizeof(kal_uint16));
...
static int open(struct subdrv_ctx *ctx)
{
	...
	rembrandtimx582_workqueue = create_setting_workqueue("rembrandtimx582_workqueue");
	enable_async = 1;
...
static int close(struct subdrv_ctx *ctx)
{
	...
	destroy_setting_workqueue(rembrandtimx582_workqueue);
	return ERROR_NONE;
} /* close */
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (last_sensor_awb.ABS_GAIN_GR != 0 &&
		...
		rembrandtimx582_awb_gain(ctx, &last_sensor_awb);
	}
	return ERROR_NONE;
}	/* custom4 */
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(rembrandtimx582_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)rembrandtimx582_ana_gain_table,
			sizeof(rembrandtimx582_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		IMX582_LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		rembrandtimx582_get_pdaf_reg_setting(ctx,
...
break;
	...
	case XIAOMI_FEATURE_LOCK_SETTING_WORK_QUEUE:
		IMX582_LOG_INF("lock setting work queue +");
		lock_setting_work(rembrandtimx582_workqueue);
		IMX582_LOG_INF("lock setting work queue -");
		break;
	case XIAOMI_FEATURE_UNLOCK_SETTING_WORK_QUEUE:
		IMX582_LOG_INF("unlock setting work queue +");
		unlock_setting_work(rembrandtimx582_workqueue);
		IMX582_LOG_INF("unlock setting work queue -");
		break;
	...
	case SENSOR_FEATURE_SEAMLESS_SWITCH:
		...
		if (pSetSensorAWB != NULL) {
			rembrandtimx582_awb_gain(ctx, pSetSensorAWB);
...
const struct subdrv_entry rembrandtimx582_mipi_raw_entry = {
	.name = "rembrandtimx582_mipi_raw",
	.id = REMBRANDTIMX582_SENSOR_ID,
	.pw_seq = pw_seq,
